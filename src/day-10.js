...
export function getPrevDir(prevCoords, currCoords, currChar) {
  ...
  switch (`${deltaX},${deltaY}`) {
    case '1,0':
      return 0;
    case '0,-1':
      if (currChar === '7') return 1;
      return 0;
    case '0,1':
      if (currChar === 'L') return 0;
      return 1;
    default:
      return 1;
  }
...
export function getNextCoords(prevCoords, currCoords, pipes) {
  ...
  // 0 === W or S, 1 === E or N
  let prevDir = getPrevDir(prevCoords, currCoords, currChar);
  const deltas = getDelta(currChar);
  const delta = deltas[prevDir];
...
export function getStartPipePair(arr, startCoords) {
  ...
  const coords = [];
  if (['|', 'F', '7'].includes(arr[y - 1]?.[x])) {
    coords.push({ x, y: y - 1 });
  }
  if (['|', 'J', 'L'].includes(arr[y + 1]?.[x])) {
    coords.push({ x, y: y + 1 });
  }
  if (['-', 'L', 'F'].includes(arr[y][x - 1])) {
    coords.push({ x: x - 1, y });
  }
  if (['-', 'J', '7'].includes(arr[y][x + 1])) {
    coords.push({ x: x + 1, y });
  }

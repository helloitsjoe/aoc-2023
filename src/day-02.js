...
function getPower(games) {
  return games.reduce((acc, { maxCubes }) => {
    const { red, blue, green } = maxCubes;
...
export default function main() {
  ...
  const games = lines.map((l) => {
    const [, id, game] = l.match(/Game (\d+): (.+)/);
    const maxCubes = game.split('; ').reduce(
      (acc, curr) => {
        const groups = curr.split(', ');

        for (const g of groups) {
          const [, count, color] = g.match(/(\d+) (\w+)/);
    ...
    return { id: Number(id), maxCubes };
  });

  console.log('games', games);

  return CHALLENGE_NUM === 1 ? sumGameIds(games) : getPower(games);
}
